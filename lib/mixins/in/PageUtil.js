...
function isAppReady(comp) {
  const APP = getApp(comp);
  const ok = (APP.rep && APP.rep.isReady) ? APP.rep.isReady() : true; // true兼容
  if (!ok) {
    console.log('WARN: APP IS NOT READY');
  }
  return ok;
}

function readGlobal(comp) {
  if (comp.APP || !isAppReady(comp)) {
    return; // 已设置过了，直接返回.
  }
  const APP = getApp(comp);
  ...
  if (!GD.session) {
    console.log('System is still Initing. See you later.');
    return;
  }
